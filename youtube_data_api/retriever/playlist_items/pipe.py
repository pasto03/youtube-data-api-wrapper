...
class PlaylistItemsPipe:
    """
    obtain playlistItems by a playlistId
    Only PlaylistItemsWorker is supposed to implement this object
    """
    def __init__(self, params: PlaylistItemsParams, playlist_items_fn=None, 
                 retrieval: RetrieveMethod = "head", n=10):
        self.params = params
        self.playlist_items_fn = playlist_items_fn
        self.retrieval = retrieval
        self.n = n
        self.hard_limit = 50
        
    def _get_response(self, pageToken=None, n=None) -> dict:
        if not self.playlist_items_fn:
            raise ValueError("Do not implement this object explicitly. Use PlaylistItemsWorker instead.")
        
        self.params.pageToken = pageToken
        self.params.maxResults = 50 if not n else n
#         print(self.params)
        request = self.playlist_items_fn.list(**self.params.to_dict())
...
#             print("nextPageToken: {}".format(nextPageToken))
    ...
    def run_pipe(self) -> list[dict] | None:
        """call API and return list of playlist items"""
